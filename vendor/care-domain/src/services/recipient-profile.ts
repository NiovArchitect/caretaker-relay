...
export function answerClinicalRetrieve(
  ...
  switch (domain) {
    ...
    case "oxygen": {
      ...
      return `No verified oxygen order or oxygen-use record is currently on file for ${name}. That is not the same as “not on oxygen” if a clinician has ordered it elsewhere — check Health & Care Details or confirm with the clinical team.`;
    }
    case "surgeries":
    ...
    case "therapies": {
      ...
      return `No verified current therapy plan is recorded for ${name}. Check Today for PT/OT times, or open Health & Care Details when therapy goals are documented.`;
    }
    case "comorbidities":
    ...
    case "code_status": {
      ...
      if (docs.length) {
        ...
        return (
          ...
          lines.join("\n") +
          `\n\nA POLST is a portable medical order when valid in the applicable state. An advance directive is broader. Only “verified medical order” is treated as an order.`
        );
      }
    ...
    case "orientation": {
      ...
      return `No verified orientation or cognitive baseline is currently recorded for ${name}. This is an information gap — not a charting action. An authorized person can add baseline notes or request them from the clinical team.`;
    }
    case "mobility":
